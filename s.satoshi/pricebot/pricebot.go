...
func (pb *PriceBot) getPrices(ctx context.Context) []*PriceBotPrice {
...
func (pb *PriceBot) PostLatestPrices(ctx context.Context) error {
	prices := pb.getPrices(ctx)
	msg := buildDiscordMessageFromPrices(prices)
	if msg == "" {
		return terrors.BadResponse("no-prices-retreived", "Failed to retrieve prices", nil)
	}
	err := pb.dc.PostToChannel(ctx, pb.discordChannel, msg)
	if err != nil {
		return terrors.Augment(err, "Failed to post prices to discord", nil)
	}
	slog.Trace(ctx, "Price bot posted to discord.", nil)
	return nil
}
